...
> This repo started from [nerves_examples/hello_live_view](https://github.com/nerves-project/nerves_examples/tree/main/hello_live_view)
> as a LiveView playground. The `rpi5` target points at
> [lawik/nerves_system_rpi5](https://github.com/lawik/nerves_system_rpi5/tree/recomputer-r22)
> branch `recomputer-r22` (Seeed reComputer R22xx: R22 overlay, CM5, Hailo-8-ready
> kernel). The target system runs OTP 29, so build with a matching host, e.g.:
>
> ```bash
> mise exec erlang@29.0.5 elixir@1.20.3-otp-29 -- \
>   env MIX_ENV=prod MIX_TARGET=host mix do deps.get + assets.setup + assets.deploy
> mise exec erlang@29.0.5 elixir@1.20.3-otp-29 -- \
>   env MIX_ENV=prod MIX_TARGET=rpi5 mix do deps.get + firmware
> ```
